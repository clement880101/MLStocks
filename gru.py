...
class Rnn:
    ...
    def structure(self, layers, units_for_layers, dropouts_for_layers):
        ...
        self.model.add(
            GRU(units_for_layers[0], activation='relu', return_sequences=True, input_shape=(self.rows, self.columns)))
        ...
        if layers > 2:
            for i in range(1, layers):
                ...
                self.model.add(GRU(units_for_layers[i], activation='relu', return_sequences=return_setting))
...
def main():
    ...
    units = [30, 30, 30, 30, 30]  # nodes for each layer
    ...
    # train model
    # nnet.train(xTrain, yTrain, epochs=10, batch_size=100, optimizer='adabound', ada_low_lr=1e-3, ada_high_lr=0.1)
    # optional adam model
    nnet.train(xTrain, yTrain, epochs=25, batch_size=100, optimizer='adam')
    ...
    plt.title('Scope=60 Testing')
    plt.xlabel('time [days]')
    plt.ylabel('price')
    plt.legend(loc='best')
    plt.show()

    y_pred = nnet.predict(xTrain)
    y_pred = y_pred * upscale_value
    yTrain = yTrain * upscale_value
    plt.plot(yTrain, color='black', label='Actual')
    plt.plot(y_pred, color='red', label='Predict')
    plt.title('Scope=60 Training')
    plt.xlabel('time [days]')
    plt.ylabel('price')
    plt.legend(loc='best')
    plt.show()
